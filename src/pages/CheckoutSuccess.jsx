...
import { CheckCircleFill, ArrowRight, Receipt, XCircleFill } from 'react-bootstrap-icons';
...
export function CheckoutSuccess() {
  ...
  useEffect(() => {
    const token = searchParams.get('_ptxn') || searchParams.get('token');
    const planType = searchParams.get('planType') || 'pro_monthly';

    if (!token) {
      setStatus('done');
      setMessage('Your payment was successful. Your account has been upgraded to Pro, and your 7-day free trial is active.');
      return;
    }
    ...
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="card-premium max-w-md w-full p-10 text-center space-y-8 animate-in zoom-in-95 duration-500 shadow-2xl shadow-primary/20 border-primary/20">
        <div className="flex justify-center">
          {status === 'error' ? (
            <div className="w-24 h-24 rounded-[2.5rem] bg-red-500/10 flex items-center justify-center shadow-inner animate-in zoom-in-50 duration-700 delay-150">
              <XCircleFill className="w-12 h-12 text-red-500" />
            </div>
          ) : (
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-black text-gradient uppercase tracking-tight">
            {status === 'error' ? 'Upgrade Failed' : 'Upgrade Successful'}
          </h1>
